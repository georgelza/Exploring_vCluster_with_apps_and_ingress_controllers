...
```bash
# ============================================
# STEP 10: Create Ingress & Middleware for NGINX 1, 2 & 3 serviced by Traefik1 
# ============================================
...
```bash
echo "Creating Ingress for NGINX 3 via Traefik1..."
cat <<EOF | kubectl apply -f -
---
apiVersion: networking.k8s.io/v1
kind: Ingress
metadata:
  ...
  namespace: webstack2
  annotations:
    traefik.ingress.kubernetes.io/router.middlewares: webstack2-stripprefix3@kubernetescrd
spec:
  ingressClassName: traefik1
  rules:
    - host: apps.local
...
# webstack2-stripprefix3 means namespace webstack2, middleware named stripprefix3.
# ============================================
echo "Creating Middleware for app1, 2 and 3"
cat <<EOF | kubectl apply -f -
---
...
kubectl wait --for=condition=ready pod -l app=nginx3 -n webstack2 --timeout=60s
```
...
echo "Adding test content to NGINX 3..."

kubectl exec -it -n webstack2 $(kubectl get pod -l app=nginx3 -o jsonpath='{.items[0].metadata.name}' -n webstack2) -- sh -c 'echo "<h1>NGINX 3 - Traefik1 Path: /app3/</h1><p>App 3</p>" > /usr/share/nginx/html/index.html'
```
...
echo "  - http://apps.local:8080/app3/    (NGINX 3 via Traefik1)"
echo ""
...
wget -qO- http://nginx3-svc.webstack2.svc.cluster.local/
...
wget -qO- --header="Host: apps.local" http://traefik1.ingress-traefik1.svc.cluster.local/app3/

```

```bash
# From Another Pod's Application:
# If you had a backend service in webstack1 that needs to call the nginx1 API:
  ...
  template:
    spec:
      ...
      - name: app
        ...
        - name: NGINX3_URL
          value: "http://nginx3-svc.webstack2.svc.cluster.local"  # backend-api is in different namespace, so we use FQDN
EOF
```
